...
<div class="container">
  ...
  <div class="dl-wrap">
    ...
    <button v-on:click="fileOutput('csv')">詳細データ（csv）</button>
  </div><!--dl-wrap end-->
  ...
  <fieldset id="genre" v-bind:class="{'active':isActive}">
    <legend>ジャンル</legend>
    <div>
  ...
  <div v-show="!msg" id="list-body" v-bind:class="displayLayout">
    ...
    <div v-for="(item,index) in getLists" v-bind:key="index" class="list-body-contents" v-bind:class="getClass(item.magazineID)">
      ...
      <div v-show="displayLayout=='list'">
        ...
        <span v-show="item.new=='1'">{{item.browsePossible}}より</span>
      </div>
...
export default {
  ...
  async mounted() {
    ...
    //設定日以降で絞り込み
    const today = new Date(this.config.date);
    var child=this.childAll;
    const filteredChild = child.filter(child => new Date(child.release) >= today);

    //子データをmagazineIDでグループ化し、distributionStartsの配列を作成
    var addChild=filteredChild.reduce((acc, item) => {
    ...
    var data = _.filter(this.all, mag => mag.magazineID !== 'delete');//deleteを除く
    var newData=data.map(item => {
      var distributionStarts = addChild[item.magazineID] || [];
      // 検索フィールドを作成
      var tags = `${item.title} ${item.titleYomi} ${item.publisher}`;
      // itemにdistributionStartsとtagsを追加して返す
      return { ...item, distributionStarts: distributionStarts, tags: tags };
    });
  ...
  methods: {
    ...
    fileOutput:function(type){
      ...
      const renderDataLib=orderData.map((data,index) =>{
        ...
        //新規
        if(data.new==1){
          newText='　 '+data.browsePossible+'より';
        }else{
          newText='';
        }
        //新規太字
